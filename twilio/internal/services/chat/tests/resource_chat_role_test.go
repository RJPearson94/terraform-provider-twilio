...
func TestAccTwilioChatRole_invalidType(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      testAccTwilioChatRole_basic(friendlyName, roleType, permissions),
				ExpectError: regexp.MustCompile("config is invalid: expected type to be one of \\[channel deployment\\], got test"),
			},
		},
	})
}
...
func testAccCheckTwilioChatRoleDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if _, err := client.Service(rs.Primary.Attributes["service_sid"]).Role(rs.Primary.ID).Get(); err != nil {
