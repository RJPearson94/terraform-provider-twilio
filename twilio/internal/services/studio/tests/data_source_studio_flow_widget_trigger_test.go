		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(stateDataSourceName, "json", `{"name":"Trigger","properties":{},"transitions":[{"event":"incomingCall"},{"event":"incomingMessage"},{"event":"incomingRequest"}],"type":"trigger"}`),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(stateDataSourceName, "json", `{"name":"Trigger","properties":{"offset":{"x":10,"y":20}},"transitions":[{"event":"incomingCall","next":"Next"},{"event":"incomingMessage","next":"Next"},{"event":"incomingRequest","next":"Next"}],"type":"trigger"}`),
					helper.ValidateFlowWidgetTrigger(stateDataSourceName),
				),
			},
		},
