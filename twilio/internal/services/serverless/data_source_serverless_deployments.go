...
func dataSourceServerlessDeployments() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceServerlessDeploymentsRead,
...
func dataSourceServerlessDeploymentsRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if utils.IsNotFoundError(err) {
			return fmt.Errorf("[ERROR] No deployments were found for serverless service with sid (%s) and environment with sid (%s)", serviceSid, environmentSid)
		}
