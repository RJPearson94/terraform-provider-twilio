...
func dataSourceChatChannel() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceChatChannelRead,
...
func dataSourceChatChannelRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if twilioError, ok := err.(*sdkUtils.TwilioError); ok {
			...
			if (twilioError.Status == 403 && twilioError.Message == "Service instance not found") || twilioError.IsNotFoundError() {
				return fmt.Errorf("[ERROR] Channel with sid (%s) was not found for chat service with sid (%s)", sid, serviceSid)
			}
		}
