...
func TestAccDataSourceTwilioAutopilotTaskField_sid(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceTwilioAutopilotTaskField_sid(uniqueName, fieldType),
...
func TestAccDataSourceTwilioAutopilotTaskField_uniqueName(t *testing.T) {
	stateDataSourceName := fmt.Sprintf("data.%s.task_field", taskFieldDataSourceName)
	uniqueName := acctest.RandString(10)
	fieldType := "Twilio.YES_NO"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceTwilioAutopilotTaskField_uniqueName(uniqueName, fieldType),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(stateDataSourceName, "unique_name", uniqueName),
					resource.TestCheckResourceAttr(stateDataSourceName, "field_type", fieldType),
					resource.TestCheckResourceAttrSet(stateDataSourceName, "id"),
...
func testAccDataSourceTwilioAutopilotTaskField_sid(uniqueName string, fieldType string) string {
	return fmt.Sprintf(`
...
func testAccDataSourceTwilioAutopilotTaskField_uniqueName(uniqueName string, fieldType string) string {
	return fmt.Sprintf(`
resource "twilio_autopilot_assistant" "assistant" {
  unique_name = "%[1]s"
}
...
data "twilio_autopilot_task_field" "task_field" {
  assistant_sid = twilio_autopilot_task_field.task_field.assistant_sid
  task_sid      = twilio_autopilot_task_field.task_field.task_sid
  unique_name   = twilio_autopilot_task_field.task_field.unique_name
}
`, uniqueName, fieldType)
}
